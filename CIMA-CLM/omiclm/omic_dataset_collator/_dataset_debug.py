...
class OmicIterableDataset(torch.utils.data.IterableDataset):
    ...
    def _process_dataset(self, i_seq_token_ds):
        ...
        return dict(
            ...
            sample_record_id=_seq_ds["record_id"],
        )
...
class OmicDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        print("_cell_embedding")
        self.dataset_i = self.dataset_i + 1
        print(f"dataset_i: {self.dataset_i}")
...
class OmicDataCollator:
    ...
    def __init__(
        ...
        if collator_i is not None:
            self.collator_i = collator_i
    ...
    def __call__(self, examples: List[InputDataClass]):
        ...
        print(batch)
        self.collator_i = self.collator_i + 1
        print(f"collator_i: {self.collator_i}")
        return batch
