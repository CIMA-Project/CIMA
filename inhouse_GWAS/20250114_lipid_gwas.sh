...
covar=/CIMA/Data/GWAS/covar_lipid.txt

LIPID=$1
pheno=/CIMA/Data/GWAS/lipid/lipid_${LIPID}.txt
out=/CIMA/CIMA_r1/Result/GWAS_lipid/lipid_${LIPID}_378

${gcta} --mlma --bfile ${bfile} --grm ${grm} --qcovar ${qcovar} --covar ${covar} --pheno ${pheno} --thread-num 1 --out ${out} && \
echo "** lipid_${LIPID} DONE **"
