...
barplot <- ggplot(eQTL_df, aes(
  ...
  labs(x = "celltype", y = "Number of Genes in cis-eQTL detection")+
...
#画caQTL
caQTL_df = read.csv('/CIMA/Data/20241230_xQTL_cell_sample_number/20241230_celltype_70_scATAC.csv')
caQTL_df$sample_1_count = 0
caQTL_df$gene_count = 0

for (celltype in caQTL_df$final_annotation){
  phenotype_df <- fread(glue('/CIMA/Data/caQTL/pseudobulk/{celltype}.csv'))
  caQTL_df[caQTL_df$final_annotation == celltype,"sample_1_count"] <- nrow(phenotype_df)
  caQTL_df[caQTL_df$final_annotation == celltype,"gene_count"] <- (ncol(phenotype_df)-1)
  rm(phenotype_df)
}

caQTL_df <-merge(caQTL_df,L1_L4_mapping,by.x = 'final_annotation',by.y = 'celltype' )
caQTL_df$final_annotation <- factor(caQTL_df$final_annotation,levels = plotting_sort$celltype)
caQTL_df$category =  factor(caQTL_df$category , levels = c("CD4 T", "CD8 T & unconvensional T", "NK&ILC", "B", "Myeloid"))

# 创建柱形图_gene_counts
barplot <- ggplot(caQTL_df, aes(
  x = final_annotation,             
  y = gene_count,  
  fill = category)) +
  geom_bar(stat = 'identity')+                                # 画柱形图                                             # x轴与y轴互换位置
  ...
  labs(x = "celltype", y = "Number of Peaks in cis-caQTL detection")+
  scale_fill_manual(values = c('#237b9f','#8ed1e5','#cca9cf','#f8b945','#91bd4a'))+
  theme(axis.text.x = element_text(angle = 90, hjust = 1),legend.position = "None")
barplot

ggsave("/CIMA/Result/plot/basic_number/Peaks_caQTL_detection.pdf", plot = barplot, width = 12, height = 4, dpi = 300, device = "pdf")
...
barplot <- ggplot(caQTL_df, aes(
  ...
  labs(x = "celltype", y = "Number of samples in cis-caQTL detection")+
  scale_fill_manual(values = c('#237b9f','#8ed1e5','#cca9cf','#f8b945','#91bd4a'))+
  theme(axis.text.x  = element_blank(),legend.position = "None")
barplot

ggsave("/CIMA/Result/plot/basic_number/samples_caQTL_detection.pdf", plot = barplot, width = 12, height = 1, dpi = 300, device = "pdf")
