...
celltype_eQTL = c('CD4_T','CD8_T','NK','Myeloid','B')

for (celltype in celltype_eQTL) {
  
  # 根据细胞类型构建第一个文件路径（假设文件路径为 '/pseudobulk' 文件夹）
  pseudobulk_matrix_path1 <- paste0('/CIMA/Data/eQTL_L1/pseudobulk/', celltype, '.csv')
  ...
  output_path1 <- paste0('/CIMA/Data/eQTL_L1/normal_dis/', celltype, '.csv')
  write.csv(expr1, file = output_path1)
  
  # 根据细胞类型构建第二个文件路径（假设是 '/top2000_pseudobulk' 文件夹）
  pseudobulk_matrix_path2 <- paste0('/CIMA/Data/eQTL_L1/top2000_pseudobulk/', celltype, '.csv')
  ...
  output_path2 <- paste0('/CIMA/Data/eQTL_L1/top2000_normal_dis/', celltype, '.csv')
