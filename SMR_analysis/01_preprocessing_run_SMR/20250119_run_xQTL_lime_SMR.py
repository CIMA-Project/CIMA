...
#OPERA_output_dir_1 = '/CIMA/Result/xQTL_GWAS_OPERA/stage_1/lipid_meta/'
#OPERA_output_dir_2 = '/CIMA/Result/xQTL_GWAS_OPERA/stage_2/lipid_meta/'
SMR_output_dir = '/CIMA/Result/xQTL_GWAS_SMR/lipid_meta/'
#xQTL_list_dir = '/CIMA/Data/xQTL_list_for_opera/'

GWAS_dir = '/CIMA/Data/inhouse_GWAS_for_SMR_OPERA/'
...
trait_all = pd.concat([trait_lipid,trait_meta])
trait_all = trait_all['V2'].to_list()
...
for trait in trait_all:
    ...
    if celltype in eQTL_CT :
        gi = f'{smr} --bfile {bim} --gwas-summary {GWAS_dir}{trait}.ma --beqtl-summary {eQTL} --thread-num 1  --out {SMR_output_dir}{trait}/eQTLsmr_{celltype}'
        os.system(gi)

    #caQTL
    if celltype in caQTL_CT :
        gi = f'{smr} --bfile {bim} --gwas-summary {GWAS_dir}{trait}.ma --beqtl-summary {caQTL}  --thread-num 1  --out {SMR_output_dir}{trait}/caQTLsmr_{celltype}'
        os.system(gi)